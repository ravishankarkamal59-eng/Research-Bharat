...
app.post("/api/articles", checkAuth, (req, res) => {
    ...
    const newArticle = {
        ...
        keyPoints: Array.isArray(keyPoints) ? keyPoints.map(point => String(point).trim()).filter(Boolean) : []
    };
...
app.put("/api/articles/:id", checkAuth, (req, res) => {
    ...
    articles[index] = {
        ...
        date: String(req.body.date || old.date || new Date().toISOString().split("T")[0]).trim(),
        tags: Array.isArray(req.body.tags) ? req.body.tags.map(tag => String(tag).trim()).filter(Boolean) : (old.tags || []),
        keyPoints: Array.isArray(req.body.keyPoints) ? req.body.keyPoints.map(point => String(point).trim()).filter(Boolean) : (old.keyPoints || [])
    };
...
app.get("/article/:id", (req, res) => {
    res.sendFile(
        path.join(__dirname, "../frontend/article.html")
    );
});
